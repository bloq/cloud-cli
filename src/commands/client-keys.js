...
class ClientKeysCommand extends Command {
  async run () {
    ...
    consola.warn('This command will generate a new pair of client keys, if you continue your current keys will be invalidated.')

    const { confirmation, save } = await inquirer.prompt([
      { name: 'confirmation', message: 'Do you want to continue?', type: 'confirm' },
      { name: 'save', message: 'Do you want that blc stores your keys locally for future usage?', type: 'confirm' }
    ])
